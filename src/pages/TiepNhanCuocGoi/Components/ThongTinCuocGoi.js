...
import { Row, Col, Avatar, Dropdown, Button, Menu, Typography, Modal } from "antd";
...
const ThongTinCuocGoi = ({ onPage, page, disableBtn = false }) => {
  const dispatch = useDispatch();

  const [profile, setProfile] = useState({});
  const {phoneNumber,client,status} = useSelector((state) => state.call)
  const [visibleDsHoSo, setVisibleDsHoSo] = useState(true);
  useEffect(() => {
    status === phoneStatus.end_call && (setVisibleDsHoSo(false))
  },[status])
      ...
      <Col span={18} style={{ padding: "12px 0 12px 12px" }}>
          {profile && Object.keys(profile).length !== 0 ?
                    ...
                    <Col span={3} style={{textAlign:"end"}}>
                      <Dropdown overlay={<ChonHoSo/>} trigger="click" disabled={disableBtn}>
        ...
        <Modal visible={visibleDsHoSo} footer={null} ><ChonHoSo phoneNumber={phoneNumber} onVisible={setVisibleDsHoSo} onFillInfo={()=>onPage(arrBtn[0].KEY)} /></Modal>
    </Row>
  );
}
